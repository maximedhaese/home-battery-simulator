    ...
    with st.spinner("Processing asset configuration models over 15-minute intervals..."):
        try:
            ...
            for visual_name, bounds in sens_vars.items():
                ...
                # Bereken impact van een verhoogde waarde (High parameter)
                if bounds["var"] == "efficiency":
            ...
            # Sorteer zodat de meest impactvolle variabele bovenaan staat (klassieke Tornado)
            df_oat = pd.DataFrame(oat_records).sort_values("Absolute_Spread", ascending=True)
            
            # Teken de ergonomische Tornado Chart (Blauw & Oranje)
            tornado = go.Figure()
            
            # Lagere waarde = Ergonomisch zacht blauw (#4A90E2)
            ...
            # Hogere waarde = Ergonomisch warm zacht oranje (#FF9F43)
            tornado.add_trace(go.Bar(
            ...
            tornado.update_layout(
                ...
                yaxis=dict(autorange="reversed"), # Zorgt dat de grootste spreiding bovenaan staat
                title=f"Tornado Sensitivity Matrix (Baseline Payback Pivot: {baseline_val:.2f} Years)",
                legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5)
            )
            
            # Gridlines stylen voor donkere achtergrond
