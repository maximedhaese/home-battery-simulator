...
def simulate_battery_roi(
    ...
    for inj, off in zip(df['historic_injection_kwh'], df['historic_offtake_kwh']):
        net_energy = inj - off 
    ...
    annual_orig_offtake = df['historic_offtake_kwh'].sum() / years_in_dataset
    annual_orig_injection = df['historic_injection_kwh'].sum() / years_in_dataset
    ...
    # Subplot 1
    fig.add_trace(go.Scatter(x=df_slice['timestamp'], y=df_slice['estimated_production_kwh'], name='Estimated Solar Gen.', line=dict(color='#ff7f0e', width=2), fill='tozeroy', legend='legend'), row=1, col=1)
    ...
    # Subplot 2: Gecorrigeerde regel hier (:= is weg)
    fig.add_trace(go.Scatter(x=df_slice['timestamp'], y=df_slice['historic_injection_kwh'], name='Orig. Injection (No Bat)', line=dict(color='rgba(44, 160, 44, 0.65)', dash='dot', width=2.5), legend='legend2'), row=2, col=1)
    fig.add_trace(go.Scatter(x=df_slice['timestamp'], y=df_slice['historic_offtake_kwh'], name='Orig. Offtake (No Bat)', line=dict(color='rgba(214, 39, 40, 0.65)', dash='dot', width=2.5), legend='legend2'), row=2, col=1)
